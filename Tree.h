...
public:
	...
	TreeNode<NODETYPE> *get_root();                            //указатель на корень
	void inorder_walk(TreeNode<NODETYPE>*);                    //печать по возрастанию
...
int Tree<NODETYPE>::insert_node(const NODETYPE &x)
{
	...
	ptr = root;
	while (ptr != 0)                     // идем от корня и ищем свободное место для нашего нового элемента
	{
		ptr1 = ptr;                 // будущий родитель вставляемого узла 
		if (x < ptr->get_data())  
			ptr = ptr->left;
		else
			ptr = ptr->right;   
...
TreeNode<NODETYPE>* Tree<NODETYPE>::delete_node(TreeNode<NODETYPE> *z)
{
	...
	if (y != z)                                        // когда мы вырезали вершину, то ее данные помещаем в z */
		z->data = y->get_data();
	return y;
}
...
{
	TreeNode<NODETYPE>* x = find(root, val);                     // получим указатель на node с значением val 
	TreeNode<NODETYPE>* y;
	if (x == 0)
		return 0;
	if (x->right != 0)                                                // если у нее есть правые потомки, то следующий элемент - минимальный в правом поддереве 
